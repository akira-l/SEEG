...
def train_beat_gan(args, epoch, time_seq, in_text, in_audio, target_poses, vid_indices,
    ...
    if epoch > warm_up_epochs and args.loss_gan_weight > 0.0:
        ...
        beat_out, *_ = pose_decoder(pre_seq, time_seq, in_text, in_audio, vid_indices)  # out shape (batch x seq x dim)
    ...
    # decoding
    out_beat, z, z_mu, z_logvar = pose_decoder(pre_seq, time_seq, in_text, in_audio, vid_indices)
    ...
    out_beat_vec_rand_vid, z_rand_vid, _, _ = pose_decoder(pre_seq, time_seq, in_text, in_audio, rand_vids)
    beta = 0.05
    ...
    ret_dict = {'loss': args.loss_regression_weight * beat_huber_loss.item()}
    if kld:
        ret_dict['KLD'] = args.loss_kld_weight * kld.item()
    if beat_div_reg:
        ret_dict['DIV_REG'] = args.loss_reg_weight * beat_div_reg.item()

    if epoch > warm_up_epochs and args.loss_gan_weight > 0.0:
        ret_dict['gen'] = args.loss_gan_weight * beat_error.item()
        ret_dict['dis'] = 0 #all_error.item()
    return ret_dict
