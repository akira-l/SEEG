...
def main(mode, checkpoint_path, option, data_version=2):
    ...
    if mode == 'eval':
        ...
        eval_net_path = 'output/train_beat_with-cat_emb_200ep/multimodal_context_checkpoint_best.bin'
        ...
        # make a video
        save_path = 'output/generation_results_syn_beat'
        ...
        with lmdb_env.begin(write=False) as txn:
            ...
            for key in keys: 
                ...
                for clip_idx in range(n_clips):
                    ...
                    #    save_path, 0, filename_prefix_for_video, str(save_counter), target_dir_vec, out_dir_vec,
                    #    mean_dir_vec, sentence, audio=clip_audio, aux_str=aux_str,
                    #    clipping_to_shortest_stream=True, delete_audio_file=False)


                    create_video_and_save_single(
                        save_folder_name, 'gt_' + str(save_counter), '',
                        target_dir_vec,
                        mean_dir_vec, sentence, audio=clip_audio, aux_str=aux_str,
                        clipping_to_shortest_stream=True, delete_audio_file=False, save_flag=True)

                    create_video_and_save_single(
                        save_folder_name, 'pred_' + str(save_counter), '',
                        out_dir_vec,
                        mean_dir_vec, sentence, audio=clip_audio, aux_str=aux_str,
                        clipping_to_shortest_stream=True, delete_audio_file=False, save_flag=True)
                    ...
                    #        torch.save(target_dir_vec[start_ind:(start_ind+stay_ind)], 
                    #                   os.path.join(sub_folder_save_name, sub_duration_str+'.pt') )
 

                    #    create_video_and_save_single(
                    #        sub_folder_save_name, str(candi_ind_), '',
                    #        target_dir_vec,
                    #        mean_dir_vec, sentence, audio=clip_audio, aux_str=aux_str,
                    #        clipping_to_shortest_stream=True, delete_audio_file=False, save_flag=True)
