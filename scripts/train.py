...
from train_eval.train_semantic import train_semantic_gan 
...
from model.beat_model import BeatGenerator, BeatDiscriminator, BeatConvDiscriminator, BeatLoopDiscriminator    
...
def train_epochs(args, train_data_loader, test_data_loader, lang_model, pose_dim, speaker_model=None):
    ...
    for epoch in range(args.epochs):
        ...
        for iter_idx, data in enumerate(train_data_loader):
            ...
            #count_bar = tqdm(total=len(in_audio)) 
            #save_dir = 'audio_save'
            #os.makedirs(save_dir)
            #cnt = 0 
            #for sub_beat, sub_audio, sub_text in zip(time_seq, in_audio, aux_info['word_list']): 
            #    count_bar.update(1)
            #    amp = sub_beat[0]
            #    onset = sub_beat[1]
            #    save_handle = open(os.path.join(save_dir, 'amp_'+str(cnt)+'.txt'), 'a') 
            #    for item in amp.tolist(): 
            #        save_handle.write(str(item) + '\n') 
            #    save_handle.close() 

            #    save_handle = open(os.path.join(save_dir, 'onset_'+str(cnt)+'.txt'), 'a') 
            #    for item in onset.tolist(): 
            #        save_handle.write(str(item) + '\n') 
            #    save_handle.close() 

            #    save_handle = open(os.path.join(save_dir, 'audio_'+str(cnt)+'.txt'), 'a') 
            #    for item in sub_audio.tolist(): 
            #        save_handle.write(str(item) + '\n') 
            #    save_handle.close() 

            #    save_handle = open(os.path.join(save_dir, 'text_'+str(cnt)+'.txt'), 'a') 
            #    save_handle.write(sub_text) 
            #    save_handle.close() 
